...
2. **Google Search Console** : en attente du même développeur pour l'enregistrement DNS TXT de vérification. Une chose à garder en tête : une fois que vous aurez la vérification Google Search Console en place, pensez à soumettre `sitemap.xml` depuis l'outil "Sitemaps" pour accélérer l'indexation de tout ça. Utiliser aussi l'outil de suppression pour les anciennes pages de spam (casino) encore indexées.
