...
1. **Finaliser Resend** : découverte en cours de route que les serveurs de noms du domaine pointent maintenant vers Vercel (`ns1`/`ns2.vercel-dns.com`) — les enregistrements DNS (DKIM, MX, SPF) ont donc été ajoutés directement dans Vercel aujourd'hui, sans passer par le développeur, et sont bien visibles en DNS. Vérification encore en cours côté Resend (propagation). Une fois vérifié, il faudra changer l'adresse d'expédition (`MAIL_FROM`) pour `contact@spirulinedechartreuse.com`.
2. ~~**Google Search Console**~~ **Fait** : domaine vérifié aujourd'hui (même découverte que ci-dessus : DNS géré par Vercel désormais). Reste à faire : soumettre `sitemap.xml` depuis l'outil "Sitemaps" pour accélérer l'indexation, et utiliser l'outil de suppression pour les anciennes pages de spam (casino) encore indexées.
3. **Vérifier les moyens de paiement Stripe qui apparaissent au paiement** : en testant, des moyens de paiement étrangers (Bancontact, MB WAY, Satispay, EPS) sont apparus sur la page de paiement alors qu'ils sont marqués "Désactivé" dans les réglages Stripe consultés. Piste probable : Stripe distingue souvent les réglages du **mode Test** de ceux du **mode Live** (un peu comme les clés API) — la page de réglages regardée était peut-être en mode Live, alors que le test de paiement utilisait le mode Test, qui a ses propres réglages (souvent plus permissifs par défaut). À vérifier : basculer le Dashboard Stripe en mode Test et revérifier les moyens de paiement activés à cet endroit-là, pour confirmer que seuls les moyens souhaités s'afficheront aux vrais clients en mode Live.
